...
def boilerplate_table(yyyymm:int, df, download_date, fixed_dqy, disadvantage):

    score = df.loc[('공급망 외관품질', '종합', '전사', '달성률'), "종합"]
    ppm = df.loc[('공급망 외관품질', '종합', '전사', 'PPM'), "종합"]
    dqy_info = "" if fixed_dqy else f"  ▶  전월불출수량기준({round(100-disadvantage*100)}%) 예상실적 "
...
def index_html(yyyymm: int, df, download_date, fixed_dqy=True, disadvantage=0):
    html = rf"""
        ...
        {margin_for_outlook()}
        {boilerplate_table(yyyymm, df, download_date, fixed_dqy, disadvantage)}
        {margin_for_outlook()}
