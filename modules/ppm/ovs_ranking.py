...
def overseas_rank(month: int):
    ...
    for i in tqdm(['누적', '당월']):
        ...
        ovs_df['고객사'] = [i.replace("(CN)", "") for i in ovs_df['고객사']]
        ...
        for j in ['한국', '글로벌']:
            ...
            for k in ['외관불량', '전체불량']:
                ...
                df = df_defect.pivot_table(values=['조치수량'], index=['코드', '부품협력사'], aggfunc=sum)
                ...
                df = df[['순위', '코드', '부품협력사', '조치수량', "점유율(%)", '주고객사', "주요품명"]]
...
if __name__ == "__main__":
    ...
    df_dict = overseas_rank(202108)

    writer = pd.ExcelWriter(r'spawn\test4.xlsx', engine='xlsxwriter')
    ...
    xfile = openpyxl.load_workbook(r'spawn\test4.xlsx')
    ...
    xfile.save(r'spawn\test4.xlsx')
    os.startfile(r'spawn\test4.xlsx')