...
from utils_.config import CUSTOMERS, APPEARANCE_TYPE, OVS_INVALID_TYPE, GLOBAL_CODES, \
    PART_REG_TO_SUBSTITUTE, PPRS_TO_EXCLUDE
import re


def overseas_rank(month: int, n_rank=10):
    ...
    for i in tqdm(['누적', '당월']):
        ...
        for j in ['한국', '글로벌']:
            ...
            for k in ['외관불량', '전체불량']:
                ...
                df = df.head(n_rank)
                df['순위'] = [i for i in range(1, n_rank+1)]
...
if __name__ == "__main__":
    ...
    df_dict = overseas_rank(202112, n_rank=20)
